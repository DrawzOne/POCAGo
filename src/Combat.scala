...
class Combat (val p1 : Pokemon, val p2 : Pokemon) {
    ...
    def choisirAttaque (p : Pokemon) : Attaque = {
     println("Au tour de " + p.nom + " d'attaquer")
     p.afficherAttaques()
    ...
    def combatPokemonSauvage () = {
      ...
      while(p1.pointsDeVie > 0 && p2.pointsDeVie > 0){
        ...
        if(p2.pointsDeVie > 0){
          attaque2 = p2.attaques(0)
          println(p2.nom + " sauvage attaque " + attaque2.nom) 
          p1.encaisserAttaque(attaque2)
          println("Les points de vie de " + p1.nom + " passent à " + p1.pointsDeVie)
        }
        
        if (p1.pointsDeVie == 0){
          println(p1.nom + " est KO " + p2.nom + " sauvage remporte le combat")
